...
{	//Variable declaration
	...
	char	sex			= ' ';
			//File information
	string	filePath	= "C:\\Users\\Kitty\\Desktop\\CSC125\\HW9\\", //= "C:\\Temp\\", 
			fileName	= " ",
			fileIn		= " ",
			fileOut		= " ";
	...
	if ((ptsPrev==0) && (hrsPrev==0))								//if hrs and pts both equal zero
		{gpaPrev =	0;
		gpaTot=		float(ptsTot)/hrsTot;
		}
	else if ((0<=ptsPrev && ptsPrev<=999) && (0<=hrsPrev && hrsPrev<=999))	//hrs and pts (0, 999]
		{gpaPrev =	float(ptsPrev)/hrsPrev;
		 gpaTot=		float(ptsTot)/hrsTot;
		}
		else if ((ptsPrev!= 0) && (hrsPrev==0))						//if dividing by 0 (hrs==0 but pts!=0)
			{gpaPrev =	-1;
			 gpaTot =	-1;
			}	
	if ((ptsMSU==0) && (hrsMSU==0))									//if hrs and pts both equal zero
		{gpaMSU =	0;
		 gpaTot=		float(ptsTot)/hrsTot;
		}
	else if ((0<=ptsMSU && ptsMSU<=999) && (0<=hrsMSU && hrsMSU<=999))		//hrs and pts (0, 999]
		{gpaMSU =	float(ptsMSU)/hrsMSU;
		 gpaTot=		float(ptsTot)/hrsTot;
		}
		else if ((ptsMSU!= 0) && (hrsMSU==0))						//if dividing by 0 (hrs==0 but pts!=0)
			{gpaMSU =	-1;
			 gpaTot =	-1;
			}
